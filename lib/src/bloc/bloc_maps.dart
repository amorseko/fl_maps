...
class ListMapsBloc {
  final _repository = Repository();
  final _getMapsModels = PublishSubject<GetMapsPartModels>();


  Stream<GetMapsPartModels> get getMapsModels => _getMapsModels.stream;


  getListMapsBloc(Map<String, dynamic> body,Function callback) async {
    GetMapsPartModels model = await _repository.getListMapsDetail(body: body);
    _getMapsModels.sink.add(model);
    callback(model.status, model.error, model.message, model);
  }

  dispose() {
    _getMapsModels.close();
  }
